...
of these artificial internal partitions need a unique name, which is unused, and
...
one of these syntaxes: Namely #1, but with the semantics of #2,
wouldn't we?

Given how slow the adoption of modules actually happens, does it really make
sense to keep the old semantic forever?

What if we were able to adjust the semantics of `"module M;"`?
...
compiles TU #6 just fine, of course currently with the net effect that the
interface of `M` (the PMIU) is imported twice. In general, the standard
doesn't care about duplicated imports. It doesn't even prescribe a
diagnostic.
